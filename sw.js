  ...
  event.waitUntil(
    caches.open(SHELL_CACHE)
      .then(cache => Promise.all(
        // addAll() rejects the whole batch if any single file 404s.
        // Adding one at a time keeps install resilient.
...
self.addEventListener('fetch', event => {
  ...
  // Never touch non-GET (no POSTs on this page, but be safe).
  ...
  // Ignore anything that isn't http(s) — chrome-extension:// etc.
  if (url.protocol !== 'http:' && url.protocol !== 'https:') return;

  // 1. Navigations and HTML: network-first.
  const wantsHTML =
  ...
  // 2. Google Fonts: cache-first in the runtime cache.
